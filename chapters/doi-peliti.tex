...
\note{This might also be interesting in that context:\url{https://arxiv.org/pdf/2302.10778}}
...
\begin{align}
    ...
    \exp \left\{ 
        ...
        P(\phi(t_1)^*, \phi(t_1))
        -
        \int_{t_0}^{t_2} \dd t \cdot
     \right\}.
\end{align}
%

Lastly, to measure moments, we found that we must chose $\Oh = (a^\dagger a)^K$.
Writing $ \ket{\Em(t_2)} = \int \D \phi e^{A} \ket{\phi(t_2)}  $, where $\Em$ is the path integral, but not a Fokk operator, we have
%
\begin{align}
...
So, letting a system evolve from $t_0$ to $t_1$, when it is initialized with $\hat I$, the to $t_2$, where it is perturbed, and then finally measure $\hat \Oh$ $t_3$, we get
%
\begin{align}
    &
...
\begin{itemize}
    ...
    \item Rewrite it using the probability generating function (PGF) $\Em(z, t)$, which gives the PDE $\partial_t\Em = \Ell \Em$.
    \item Use the 2nd quantization (2Q), substituting $z, \fdv{  }{ z }$ for $a^\dagger, a$, and normal order $\Ell[a^\dagger, a]$.
